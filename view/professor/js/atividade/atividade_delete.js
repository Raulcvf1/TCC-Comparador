const btnAtividadeDelete = document.getElementById("btnAtividadeDelete");

btnAtividadeDelete.onclick = onclick_btnAtividadeDelete;

function onclick_btnAtividadeDelete() {
    fetch_delete_deleteAtividade();
}

//função assincrona para validar login
function fetch_delete_deleteAtividade() {
    ...
    requisicao_assincrona.then((response) => {
        ...
        if (objetoJson.status == true) {
            ...
            const path = `C:\\workspace tcc\\corretorFinalizado\\professor\\${professor.registro}\\${disciplina.idDisciplina}\\${atividade.idAtividade}`;
